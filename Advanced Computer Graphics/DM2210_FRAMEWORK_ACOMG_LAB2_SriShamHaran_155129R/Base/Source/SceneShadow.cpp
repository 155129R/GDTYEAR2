...
void SceneShadow::Init()
{
    ...
    m_parameters[U_FOG_ENDPOINT] = glGetUniformLocation(m_programID, "fogParam.end");
    m_parameters[U_FOG_STARTPOINT] = glGetUniformLocation(m_programID, "fogParam.start");
    ...
    lights[0].position.Set(1, 12000, 4000);
    ...
    camera.Init(Vector3(0, 300, 8), Vector3(0, 0, 2), Vector3(0, 1, 0));
    ...
    meshList[GEO_QUAD]->textureArray[0] = LoadTGA("Image//sand1.tga");
    ...
    meshList[GEO_SPHERE] = MeshBuilder::GenerateSphere("sphere", Color(0.7f, 0, 1), 18, 36, 10.f);
    meshList[GEO_SPHERE]->textureArray[0] = LoadTGA("Image//NightSky.tga");
    ...
    meshList[GEO_LAVA] = MeshBuilder::GenerateQuad("lava", Color(0, 0, 0), 1);
    //meshList[GEO_LAVA]->textureArray[0] = LoadTGA("Image//lava2.tga");
    meshList[GEO_LAVA]->textureArray[1] = LoadTGA("Image//lava1.tga");
    ...
    meshList[GEO_PARTICLE_SAKURA]->material.kAmbient.Set(0.3f, 0.3f, 0.3f);
    meshList[GEO_PARTICLE_SAKURA]->material.kSpecular.Set(0.1f, 0.1f, 0.1f);
    meshList[GEO_LIGHT_DEPTH_QUAD] = MeshBuilder::GenerateQuad("LIGHT_DEPTH_TEXTURE", Color(1, 1, 1),1.f);
    meshList[GEO_LIGHT_DEPTH_QUAD]->textureArray[0] = m_lightDepthFBO.GetTexture();

    meshList[GEO_CANDLE] = MeshBuilder::GenerateOBJ("CANDLE", "OBJ\\candle.obj");
    meshList[GEO_SKULL] = MeshBuilder::GenerateOBJ("CANDLE", "OBJ\\head.obj");
    meshList[GEO_SKULL]->textureArray[0] = LoadTGA("Image//stacy.tga");
...
void SceneShadow::Update(double dt)
{
    ...
    Mtx44 lightRotation;
    lightRotation.SetToRotation(10 * dt, 1, 0, 1);
    lights[0].position = lightRotation * lights[0].position;
...
void SceneShadow::RenderTerrain()
{
    ...
    RenderMesh(meshList[GEO_LAVA], false);
    modelStack.PopMatrix();
}
...
void SceneShadow::updateParticles(double dt)
{
    if (m_particleCount < MAX_PARTICLE)
    {
        ...
        {
            float scale = Math::RandFloatMinMax(1, 4);
            particle->vel.Set(Math::RandFloatMinMax(-100, 30), Math::RandFloatMinMax(-100, 30), Math::RandFloatMinMax(-100, 30));
            particle->rotationSpeed = (Math::RandFloatMinMax(10.f, 80.f));
            particle->scale.Set(scale, scale, scale);
            float posY = 195.f + 250.f * ReadHeightMap(m_heightMap, (0) / 4000.f, (-600) / 4000.f);;
            particle->pos.Set(Math::RandFloatMinMax(-140, -40), Math::RandFloatMinMax(posY - 20, posY + 50), Math::RandFloatMinMax(-620, -595));
        }
    }
    ...
    for (std::vector<ParticleObject*>::iterator it = particleList.begin(); it != particleList.end(); ++it)
    {
        ...
        if (particle->active)
        {
            ...
            {
                particle->vel += m_gravity * (float)dt;
                particle->pos += particle->vel * (float)dt * 0.3f;
                particle->rotation += particle->rotationSpeed * (float)dt;
                if (particle->pos.y < (ReadHeightMap(m_heightMap, particle->pos.x / 4000.f, particle->pos.z / 4000.f) * 350.f))
                {
                    particle->active = false;
...
void SceneShadow::RenderParticles(ParticleObject* particles)
{
...
void SceneShadow::RenderWorld()
{
    ...
    Vector3 pos;
    pos.Set(-50, 0, -220);
    pos.y = 80.f + 250.f * ReadHeightMap(m_heightMap, pos.x / 4000.f, pos.z / 4000.f);
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        pos.Set(100 - (i*70), 0, 40);
        pos.y = 2 + 250.f * ReadHeightMap(m_heightMap, pos.x / 4000.f, pos.z / 4000.f);
        ...
        modelStack.Rotate(-35 + Math::RandFloatMinMax(-2,2), 1, 0, 1);
    ...
    modelStack.PopMatrix();
        ...
        modelStack.PushMatrix();
        pos.Set(-70 , 0, -600);
        pos.y = 250.f * ReadHeightMap(m_heightMap, pos.x / 4000.f, pos.z / 4000.f);

        modelStack.Translate(pos.x, pos.y, pos.z);
        modelStack.Scale(4, 4, 4);
        modelStack.Rotate(rotateAngle * 2, 0, 1, 0);
        RenderMesh(meshList[GEO_CANDLE], true);
        modelStack.PopMatrix();
...
void SceneShadow::RenderPassGPass()
{
    ...
    if (lights[0].type == Light::LIGHT_DIRECTIONAL)
        m_lightDepthProj.SetToOrtho(-400, 400, -400, 400, 0, 16000);
    else
    {
...
void SceneShadow::RenderPassMain()
{
    ...
    glViewport(0, 0, Application::GetInstance().m_window_width, Application::GetInstance().m_window_height);
