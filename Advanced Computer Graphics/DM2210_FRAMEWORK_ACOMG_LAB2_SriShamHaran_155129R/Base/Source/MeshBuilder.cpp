...
Mesh* MeshBuilder::GenerateSkyPlane(const std::string &meshName, Color color, int slices, float PlanetRadius, float AtmosphereRadius, float hTile, float vTile)
{
	...
	//Check whether no. of slices is within 1 - 256
		//Alternative way: (slices < 1) ? (slices = 1) : (slices > 256) ? (slices = 256) : (slices);
	Math::Clamp(slices, 1, 256);

	//Calculate some values that you need for the skyplane
...
Mesh* MeshBuilder::GenerateTerrain(const std::string &meshName, const std::string& file_path, std::vector<unsigned char>& heightMap)
{
	Vertex v;
    std::vector<Vertex> vertex_buffer_data;
    std::vector<Vertex> normal_buffer_data;
	std::vector<GLuint> index_buffer_data;
	...
	for (int z = 0; z < terrainSize; ++z){
		...
		for (int x = 0; x < terrainSize; ++x){
			...
			v.color.Set(scaledHeight, scaledHeight, 0.f);
			v.texCoord.Set((float)x / terrainSize * 8, 1.0f - (float)z / terrainSize * 8);        
            Vector3 v1, v2, p1, p2, p3, vN;
            p1.Set(v.pos.x, v.pos.y, v.pos.z);
            p2.Set(v.pos.x, v.pos.y, v.pos.z + 1);
            p3.Set(v.pos.x + 1, v.pos.y, v.pos.z + 1);
            v1 = p2 - p1;
            v2 = p3 - p1;
            vN = v1.Cross(v2);
            vN.Normalize();
            v.normal += vN;
            //v.normal.Set(static_cast<float>(x) / terrainSize - 0.5f, scaledHeight + 1, static_cast<float>(z) / terrainSize - 0.5f);
			vertex_buffer_data.push_back(v);
		}
	}
...
SpriteAnimation* MeshBuilder::GenerateSpriteAnimation(const std::string &meshName, unsigned numRow, unsigned numCol)
{
    ...
    SpriteAnimation* returnSA = new SpriteAnimation(meshName, numRow, numCol);

    glBindBuffer(GL_ARRAY_BUFFER, returnSA->vertexBuffer);
    glBufferData(GL_ARRAY_BUFFER, vertex_buffer_data.size() * sizeof(Vertex), &vertex_buffer_data[0], GL_STATIC_DRAW);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, returnSA->indexBuffer);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, index_buffer_data.size() * sizeof(GLuint), &index_buffer_data[0], GL_STATIC_DRAW);

    returnSA->indexSize = index_buffer_data.size();
    returnSA->mode = Mesh::DRAW_TRIANGLES;
