...
void SceneAssn::Init()
{
    ...
    lights[0].position.Set(0, 550, 240);
    ...
    camera.Init(Vector3(0, 0, 8), Vector3(0, 0, 2), Vector3(0, 1, 0));
...
void SceneAssn::Update(double dt)
{
...
void SceneAssn::RenderTerrain()
{
    ...
    //modelStack.Translate(0, 50, 0);
    modelStack.Scale(3800.f, 3800.f, 1.f); //values varies
    RenderMesh(meshList[GEO_WATER], false);
    modelStack.PopMatrix();
}

void SceneAssn::RenderText(Mesh* mesh, std::string text, Color color)
{
...
void SceneAssn::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y)
{
    if (!mesh || mesh->textureID <= 0)
        return;
...
void SceneAssn::RenderMeshIn2D(Mesh *mesh, bool enableLight, float size, float x, float y)
{
    Mtx44 ortho;
...
void SceneAssn::RenderMesh(Mesh *mesh, bool enableLight)
{
    Mtx44 MVP, modelView, modelView_inverse_transpose;
...
ParticleObject* SceneAssn::GetParticle(void)
{
...
void SceneAssn::updateParticles(double dt)
{
...
void SceneAssn::RenderParticles(ParticleObject* particles)
{
    ...
    case ParticleObject_TYPE::P_SAKURA:
        ...
        modelStack.Scale(particles->scale.x, particles->scale.y, particles->scale.z);
        modelStack.Rotate(angleAni, 0, 1, 0);
        RenderMesh(meshList[GEO_PARTICLE_SAKURA], true);
        modelStack.PopMatrix();
        break;
    default:
        break;
...
void SceneAssn::RenderPassGPass()
{
    ...
    if (lights[0].type == Light::LIGHT_DIRECTIONAL)
        m_lightDepthProj.SetToOrtho(-10, 10, -10, 10, 1, 20);
    else
    {
