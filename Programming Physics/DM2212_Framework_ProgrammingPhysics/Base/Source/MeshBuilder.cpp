...
*/
/******************************************************************************/
Mesh* MeshBuilder::GenerateQuad(const std::string &meshName, Color color, float length, float tileX, float tileY)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;
	...
	v.color = color;
	v.normal.Set(0, 0, 1);
	v.texCoord.Set(1.0f * tileX, 0);
	vertex_buffer_data.push_back(v);
	v.pos.Set(0.5f * length, 0.5f * length,0);
	v.color = color;
	v.normal.Set(0, 0, 1);
	v.texCoord.Set(1.0f * tileX, 1.0f* tileY);
	vertex_buffer_data.push_back(v);
	v.pos.Set(-0.5f * length, 0.5f * length,0);
	v.color = color;
	v.normal.Set(0, 0, 1);
	v.texCoord.Set(0, 1.0f * tileY);
	vertex_buffer_data.push_back(v);
...
*/
/******************************************************************************/
Mesh* MeshBuilder::GenerateCube(const std::string &meshName, Color color, float length, float tileX, float tileY)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
...
Mesh* MeshBuilder::GenerateCylinder(const std::string &meshName, Color color, float cylinderHeight, float cylinderTopWidth, float cylinderBotWidth, unsigned numSlices){
    ...
    for (unsigned short slice = 0; slice <= numSlices; ++slice){
        ...
        v.pos.Set(0, -cylinderHeight / 2, 0);
        v.color = color;
        vertex_buffer_data.push_back(v);
    ...
    for (unsigned short slice = 0; slice <= numSlices; ++slice){
        ...
        v.pos.Set(CircleX(theta)* cylinderBotWidth, -cylinderHeight / 2, cylinderBotWidth * CircleZ(theta));
        v.color = color;
        v.normal.Set(CircleX(theta), -cylinderHeight / 2, CircleZ(theta));
        vertex_buffer_data.push_back(v);
